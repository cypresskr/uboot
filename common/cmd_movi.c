...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (attribute == 0x7) {
		...
		start_blk = image[i].start_blk;
		blkcnt = image[i].used_blk;
		printf("%s xen..device %d Start %ld, Count %ld ", rw ? "writing" : "reading",
			dev_num, start_blk, blkcnt);
		sprintf(run_cmd, "mmc %s %d 0x%lx 0x%lx 0x%lx",
			rw ? "write" : "read", dev_num,
			addr, start_blk, blkcnt);
...
U_BOOT_CMD(
	...
	"movi read dtb {device_number} {addr} - Read data from device\n"
	"movi write dtb {device_number} {addr} - Write data to device\n"
	"movi read xen {device_number} {addr} - Read data from device\n"
	"movi write xen {device_number} {addr} - Write data to device\n"
