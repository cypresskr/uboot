...
int init_raw_area_table(block_dev_desc_t * dev_desc, int location)
{
	...
	/* image 6 should be DTB */
	image[6].start_blk = image[5].start_blk + MOVI_ZIMAGE_BLKCNT;
	image[6].used_blk = MOVI_DTB_BLKCNT;
	image[6].size = PART_SIZE_DTB;
	image[6].attribute = 0x6;
	strcpy(image[6].description, "device tree");
	dbg("dtb: %d\n", image[6].start_blk);

	/* image 7 should be XEN */
	image[7].start_blk = image[6].start_blk + MOVI_DTB_BLKCNT;
	image[7].used_blk = MOVI_XEN_BLKCNT;
	image[7].size = PART_SIZE_XEN;
	image[7].attribute = 0x7;
	strcpy(image[7].description, "xen");
	dbg("xen: %d\n", image[7].start_blk);

	/* image 8 should be RFS */
	image[8].start_blk = image[7].start_blk + MOVI_XEN_BLKCNT;
	image[8].used_blk = MOVI_ROOTFS_BLKCNT;
	image[8].size = PART_SIZE_ROOTFS;
	image[8].attribute = 0x8;
	strcpy(image[8].description, "rfs");
	dbg("rfs: %d\n", image[8].start_blk);
