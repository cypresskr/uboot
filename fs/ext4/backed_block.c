...
{
	struct data_block *db = ext4_malloc(sizeof(struct data_block));
	if (db == NULL)
		critical_error_errno("ext4_malloc");
...
void for_each_data_block(block_dev_desc_t *dev_desc, disk_partition_t* part, data_block_callback_t data_func,
	...
	data_block_fill_callback_t fill_func)
{
	...
	for (db = data_blocks; db; db = db->next) {
		...
		last_block = db->block + EXT4_DIV_ROUND_UP(db->len, info.block_size) - 1;

		if (db->filename)
			file_func((u64)db->block * info.block_size, db->filename, db->offset, db->len);
		else if (db->fill)
			fill_func(dev_desc, part, (u64)db->block * info.block_size, db->fill_val, db->len);
		else
			data_func(dev_desc, part, (u64)db->block * info.block_size, db->data, db->len);
	}
}
