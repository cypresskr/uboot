...
{
	if (EXT4_DIV_ROUND_UP(backing_len, info.block_size) > EXT4_NDIR_BLOCKS)
		critical_error("indirect backing larger than %d blocks", EXT4_NDIR_BLOCKS);

	u8 *data = (u8*)ext4_calloc(backing_len, 1);
	if (!data)
...
static void fill_dindirect_block(u32 *dind_block, int len, struct block_allocation *alloc)
{
	...
	for (i = 0; len >  0; i++) {
		...
		u32 *ind_block_data = (u32*)ext4_calloc(info.block_size, 1);
...
static void fill_tindirect_block(u32 *tind_block, int len, struct block_allocation *alloc)
{
	...
	for (i = 0; len > 0; i++) {
		...
		u32 *dind_block_data = (u32*)ext4_calloc(info.block_size, 1);
		...
		struct block_allocation *alloc, u32 *block_len)
{
	...
	u32 *tind_block_data = (u32*)ext4_calloc(info.block_size, 1);
...
static u32 indirect_blocks_needed(u32 len)
{
	...
	ind += EXT4_DIV_ROUND_UP(len, aux_info.blocks_per_ind);
	...
	ind += EXT4_DIV_ROUND_UP(len, aux_info.blocks_per_dind);
	...
	ind += EXT4_DIV_ROUND_UP(len, aux_info.blocks_per_tind);
...
void inode_allocate_indirect(struct ext4_inode *inode, unsigned long len)
{
	...
	u32 block_len = EXT4_DIV_ROUND_UP(len, info.block_size);
	u32 indirect_len = indirect_blocks_needed(block_len);
		...
		struct block_allocation *alloc)
{
	...
	u32 *ind_block_data = (u32*)ext4_calloc(info.block_size, info.bg_desc_reserve_blocks);
	if (!ind_block_data)
		critical_error_errno("ext4_calloc");
