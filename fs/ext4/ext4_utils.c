...
{
	alloc_ptr = (CONFIG_SYS_SDRAM_BASE + 0x40000000);
}

#if defined(CONFIG_MMC_64BIT_BUS) || defined(CONFIG_CPU_EXYNOS5410_EVT2)
void* ext4_malloc(u32 size)
{
	u32 old;

	if(alloc_ptr + size > (CONFIG_SYS_SDRAM_BASE + 0x50000000))
		return 0;

	if (((u32)alloc_ptr % 8) == 0) {
		*(u32 *)(alloc_ptr + 4) = (u32)alloc_ptr;
		alloc_ptr = (u32)(alloc_ptr + 8);
	}
	else {
		*(u32 *)(alloc_ptr) = (u32)alloc_ptr;
		alloc_ptr = (u32)(alloc_ptr + 4);
	}
...
void* ext4_calloc(u32 size, u32 cnt)
{
	u32 old;

	if(alloc_ptr + size*cnt > (CONFIG_SYS_SDRAM_BASE + 0x50000000))
		return 0;
...
static void ext4_write_data_block(block_dev_desc_t *dev_desc, disk_partition_t* part, u64 off, u8 *data, int len)
{
	write_data_block(dev_desc, part, off, data, len);
}

static void ext4_write_fill_block(block_dev_desc_t *dev_desc, disk_partition_t* part, u64 off, u32 fill_val, int len)
{
	write_fill_block(dev_desc, part, off, fill_val, len);
}

static void ext4_write_data_file(u64 off, const char *file,
		int offset, int len)
{
	write_data_file(off, file, offset, len);
}
...
void write_ext4_image(block_dev_desc_t *dev_desc, disk_partition_t* part, const char *filename, int gz, int sparse, int crc,
		int wipe)
{
	for_each_data_block(dev_desc, part, ext4_write_data_block, ext4_write_data_file, ext4_write_fill_block);
...
void ext4_create_fs_aux_info()
{
	...
	aux_info.backup_sb = ext4_calloc(aux_info.groups, sizeof(char *));
...
void ext4_free_fs_aux_info()
{
	...
	for (i=0; i<aux_info.groups; i++) {
		if (aux_info.backup_sb[i])
			ext4_free(aux_info.backup_sb[i]);
	}
...
void ext4_fill_in_sb()
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		if (ext4_bg_has_super_block(i)) {
			if (i != 0) {
				aux_info.backup_sb[i] = ext4_calloc(info.block_size, 1);
...
void ext4_queue_sb(void)
{
	...
	if (info.block_size > 1024) {
		u8 *buf = ext4_calloc(info.block_size, 1);
